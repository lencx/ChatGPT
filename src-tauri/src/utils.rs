...
pub fn create_courtney_prompts() {
  let prompts_file = app_root()
    .join("cache_prompts")
    .join("courtney_prompts.json");
  if !exists(&prompts_file) {
    create_file(&prompts_file).unwrap();
    fs::write(&prompts_file, "[]").unwrap();
  }
}
...
pub fn run_check_update(app: AppHandle<Wry>, silent: bool, has_msg: Option<bool>) {
  ...
  // Updater disabled for Courtney AI
  if let Some(v) = has_msg {
    if v {
      tauri::api::dialog::message(
        app.app_handle().get_window("core").as_ref(),
        "Courtney AI",
        "Auto-update is currently disabled. Please check manually for updates.",
      );
    }
  }
}

// Updater functionality removed - Courtney AI uses local deployment
